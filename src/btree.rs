...
pub fn get(table: &Table<B>, key: &[u8], out: &mut Vec<u8>, path: &mut Vec<PageId>) -> Option<()> {
...
pub fn set(table: &Table<B>, key: &[u8], val: &[u8], path: &mut Vec<PageId>) -> Result<(), ()> {
    find_leaf(table, key, path);
    match table
        ...
        .reserve(9 + key.len() + val.len())
    {
        ...
        ReserveResult::Sealer(seal_guard) => {
            handle_compaction(table, seal_guard, path);
            Ok(())
        }
    }
}

fn find_leaf(table: &Table<B>, key: &[u8], path: &mut Vec<PageId>) {
    path.clear();
...
fn handle_compaction(table: &Table<B>, seal_guard: SealGuard, path: &mut Vec<PageId>) {
...
fn handle_split(
    ...
    let (from_top, to_top) = split_leaf(&mut new[top..], to_buf, split_idx);
...
fn split_inner<'s>(from: &'s mut [u8], to: &'s mut [u8], idx: usize) -> (usize, usize) {
    todo!()
}
...
fn compact_inner(og: &[u8], new: &mut [u8]) -> CompactResult {
